...
// import { requestSigninPassword } from "../components/utils/StudioUtils";
...
function submit(url, payload, options) {
    if (ServiceConstants.USE_AXIOS) {
        if (options && options.contentType === 'application/octet-stream') {
            return fileReader(payload).then((fileContent) => {
                return requestProvier().post(url, fileContent, requestConfig(options)).then(handleResponse).catch(handleError);
            })
...
function update(url, payload, options) {
    ...
    if (ServiceConstants.USE_AXIOS) {
        if (options && options.contentType === 'application/octet-stream') {
            return fileReader(payload).then((fileContent) => {
                return requestProvier().put(url, fileContent, requestConfig(options)).then(handleResponse).catch(handleError);
            })
        } else if (options && options.contentType === 'multipart/form-data') {
            var multipartData = new FormData();
            Object.keys(payload).forEach(function (payloadKey) {
...
function requestConfig(options) {
    ...
    providerConfig.timeout = 60 * 1000;
    ...
    // }
    providerConfig.validateStatus = null;
...
function requestHeaders(options) {
    ...
    let authToken = options?.Authorization || userService.getAuthorizationToken(options);
    ...
    if(options && options.headers) {
        Object.keys(options.headers).forEach(header => {
            requestHeader[header] = options.headers[header];
        })
    }
